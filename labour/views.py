...
# Rozi day-type labels as the Mistri understands them.
ROZI_DAY_LABELS = {
    'FULL': 'Full Day',
    'ONE_HALF': 'Overtime',
    'HALF': 'Half Day',
...
def _labour_type_label(labour):
    """Proper role label: Mistri / Mistri Helper / Tractor Driver / ..."""
    if labour.category == 'MISTRI':
        if labour.sub_category == 'HELPER':
            return 'Mistri Helper'
        return 'Mistri'
    display = labour.get_category_display()
    if labour.is_driver and not display.endswith('Driver'):
        return f'{display} Driver'
    return display


def _rozi_rate_for(labour, day_type):
    """Per-day rate behind a rozi entry (fixed for Mistri, base rate otherwise)."""
    if labour.category == 'MISTRI' and labour.sub_category == 'MISTRI':
        return LabourRozi.MISTRI_RATES[day_type]
    return labour.base_daily_rate
...
def _labour_statement_for_period(labour, period_start, period_end):
    ...
    for rz in LabourRozi.objects.filter(
        ...
        rozi_by_date.setdefault(rz.date, {
            'day_type': rz.day_type,
            'day_label': ROZI_DAY_LABELS.get(
                rz.day_type, rz.get_day_type_display()
            ),
            'rate': _rozi_rate_for(labour, rz.day_type),
            'amount': rz.amount,
        })
...
def _labour_book_excel(statements, period_start, period_end, filename='labour_book.xlsx'):
    ...
    for st in statements:
        ...
        info = [
            ...
            ('Type', _labour_type_label(labour)),
        ...
        if is_mistri_sheet:
            summary_items = [
                ...
                ('Rozi', rozi_total, False),
                ('Extra', st['extra_total'] - rozi_total, False),
        ...
        if is_rozi_sheet:
            ...
            trip_total = Decimal('0')
            folded_days = set()
            for kind, item in _ordered_day_entries(st):
                if kind == 'group':
                    g = item
                    d = g.date
                    first_of_day = d not in folded_days
                    folded_days.add(d)
                    ...
                    desc = _trip_group_desc(labour, g)
...
def _trip_group_desc(labour, group):
    """Entries table wali work description (Hyva prefix ke saath)."""
    desc = group.load_label or 'Tractor Trip'
    if labour.category == 'HYVA_DRIVER':
...
def _ordered_day_entries(st):
    ...
    for g in sorted(st.get('trip_groups') or [], key=lambda g: (g.date, g.id)):
        groups_by_date[g.date].append(g)
    extra_rows = {
        row['date']: row for row in st['rows']
...
def _category_summary(st):
    """Load-type wise (Category | Trips | Amount) + grand trips/amount."""
    labour = st['labour']
    summary = []
    seen = set()
    grand_trips = 0
    ...
    for g in st.get('trip_groups') or []:
        n = g.labourers.count() or 1
        share = g.total_amount / n
        grand_trips += g.trip_count
        grand_amount += share
        desc = _trip_group_desc(labour, g)
        for i, (label, trips, amount) in enumerate(summary):
            if label == desc:
                summary[i] = (label, trips + g.trip_count, amount + share)
                break
        else:
